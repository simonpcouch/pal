...
Pals return output by replacing, prefixing, or suffixing the current selection. **Is this the right place for output in your use case?** If output ultimately needs to be cut and pasted into a different file, for example, helpers may not be the right tool.
...
To create a new helper, add a prompt to chores' prompt directory with `prompt_new()`. You'll need to supply a `role` (a single keyword describing what the helper does, like `"roxygen"`) and an `interface` describing how the helper will interact with the selection (one of `"replace"`, `"prefix"`, or `"suffix"`). You can also "pre-fill" the contents of the prompt by supplying a file path with the `contents` argument.
...
Pal extension packages allow you to more flexibly share helper prompts with others. Putting together a helper extension package is straightforward:
